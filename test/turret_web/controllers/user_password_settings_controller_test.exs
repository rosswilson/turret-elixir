defmodule TurretWeb.UserPasswordSettingsControllerTest do
  ...
  describe "GET /users/settings/password" do
    test "renders settings page", %{conn: conn} do
      conn = get(conn, Routes.user_password_settings_path(conn, :edit))
      response = html_response(conn, 200)
      assert response =~ "<h1>Change password</h1>"
    end
  ...
  describe "PUT /users/settings/password" do
    test "updates the user password and resets tokens", %{conn: conn, user: user} do
      new_password_conn =
        put(conn, Routes.user_password_settings_path(conn, :update), %{
